...
**Status:** MVP starter built, ready to configure against your real data
...
**One thing to do before running this for real:** open `mongosh`,
run `db.products.findOne()`, and check the actual field names against
`_project_product()` in `catalog_sync.py` (currently assumes fields
like `title`, `brand`, `sku`/`modelNumber`, `price`, `category` —
